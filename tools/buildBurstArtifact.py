#!/usr/bin/env python3
"""Bundle the burst sandbox into one self-contained page.
...
Usage:
    python3 tools/buildBurstArtifact.py        # writes artifacts/burst-bench.html
"""
...
MODULES = ['rng', 'board', 'glyphShapes', 'compositor', 'palette', 'animate', 'level', 'render']
...
def main() -> None:
    page = (ROOT / 'tools' / 'burst-artifact.template.html').read_text(encoding='utf-8')
    page = page.replace('/*__ENGINE__*/', bundle_modules())
    page = page.replace('/*__DATA__*/', bundle_data())
    non_ascii = sorted({c for c in page if ord(c) > 127})
    if non_ascii:
        raise SystemExit(f'page must be pure ASCII; found {non_ascii}')
    target = ROOT / 'artifacts' / 'burst-bench.html'
